/*
  # Fix user_roles RLS policies and infinite recursion
  ...
  2. Security
    ...
    - Create new policies using is_admin function to prevent recursion
    - Update all related table policies to use proper role checking
...
-- Drop ALL existing policies on user_roles table (comprehensive cleanup)
DO $$
DECLARE
...
-- Drop existing policies that might cause conflicts on other tables
